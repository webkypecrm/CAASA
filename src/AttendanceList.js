...
const AttendanceList = () => {
  ...
  const trackEmployeeAttandence = () => {
    navigation("/track-employee-attandance");
  };
  ...
  const fetchDataFromApi = async () => {
    setLoading(true);
    try {
      const { dates, gender, designation, type } = filterByObj;
      const url = `${apiUrl}/employee/dailyAttendance?id=&mobileNumber=&emailId=&dates=${dates}&gender=${gender}&type=${type}&designation=${designation}`;
      ...
      if (data.status === "success" && Array.isArray(data.data)) {
        ...
        setTodayDate(data.selectedDate);
      } else {
        console.error(
          "API response error:",
                          ...
                          <table className="table table-striped table-bordered text-nowrap mb-0">
                            <thead>
                              <tr>
                                ...
                                <th className="col-2">Employee Details</th>
                                <th className="col-2">Exclusion</th>
                                <th className="col-2">Attendance Location</th>
                                <th className="col-2">Status</th>

                                <th className="col-2">Employee Logs</th>

                                <th className="col-3">Actions</th>
                                <th className="col-2">Flag</th>
                                <th className="col-2">Penalty</th>
                              ...
                              {filteredUsers.map((user) => (
                                <tr>
                                  ...
                                  <td>
                                    {/* <td> */}
                                    <Link
                                      to="/track-employee-attandance"
                                      className={`btn  btn-xs  w-100 mb-1 text-white ${user.employeeTracking.length > 0
                                        ? "btn-warning"
                                        : "btn-danger"
                                        }`}
                                      state={{
                                    ...
                                    >
                                      <strong>
                                        {user.employeeTracking.length}
                                      </strong>{" "}
                                      Location
                                    </Link>
                                    <br />
                                    {/* </td> */}
                                    {user.employeeTracking.length === 0 && (
                                      <span style={{ color: "red" }}>
                                        Mobile location is disabled
                                      </span>
                                    )}
                                    <br />
                                    {user.employeeTracking.length > 0 && user.locationMismatch === true && (
                                      <span style={{ color: "red" }}>
                                  ...
                                  <td>
                                    <a
                                      // onClick={trackEmployeeAttandence}
