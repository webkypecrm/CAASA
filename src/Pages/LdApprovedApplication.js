...
`;
    ...
    const loadcontent7 = (id, fullName, middleName, lastName, user) => {
        navigation(
            `/inventorys-check/${id}`,
            {
                state: {
    ...
    const handleOpenModal56 = async (id, ids, schemeIds, schemeId, ticketId) => {
    ...
    //INVENTORY api 
    useEffect(() => {
        fetch(`${apiUrl}/inventory/inventoryDropdown?projectId=${projectId || inventoryAllocation.projectId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${Token}`,
    ...
    }, [inventoryAllocation.projectId, Token, projectId, apiUrl]);
    ...
    const fetchDataFromApi = async () => {
        ...
        const url = `${apiUrl}/applicant/applicants?projectId=${projectId}&schemeId=${schemeId}&schemeType=${schemeType}&from=${from}&to=${to}&stage=${stage}&page=${pageNumber}&limit=${itemsPerPage}&search=${search}&luckyDrawId=${luckyDrawId}&isUpdated=0&resultStatus=0&isVerified=true&isWLD=true`;
    ...
    const handleApproved = async () => {
        try {
            setLoadings(true);
            const url = `${apiUrl}/applicant/bookTheInventory?applicantId=${userId}&inventoryId=${inventoryAllocation.inventoryId}&giftId=${inventoryAllocation.giftId}&projectId=${projectId}&bsp=${employee2?.plan?.basicPriceFPP}&paymentPlan=${employee2?.paymentPlan}`;
            ...
            <div className="page">
                            ...
                            <div className="page-header">
                                <div>
                                    <h2 className="main-content-title tx-24 mg-b-5">
                                        WLD Approved Applications List ({leadCount})
                                    </h2>
                                </div>
                                                                                ...
                                                                                <tr>
                                                                                    ...
                                                                                    <td style={{ padding: '15px', textAlign: 'center' }}>
                                                                                        ...
                                                                                        <button
                                                                                            ...
                                                                                            onClick={() => loadcontent7(user.id, user.applicantFirstName, user.applicantMiddleName, user.applicantLastName, user)}
                                                                                    ...
                                                                                    <td style={{ textAlign: "center" }}>
                                                                                        ...
                                                                                        <button
                                                                                            ...
                                                                                            onClick={() => handleOpenModal56(user.projectId, user.id, user.schemeIds, user.schemeId, user.ticketId)}
                                                                                            type="button"
                                                                                            className="btn ripple btn-info btn-xs w-100 mb-1"
                                                                                            disabled
                ...
                >
                    <div className="modal-dialog modal-dialog-centered modal-xl" style={{ maxWidth: '40%', margin: 'auto' }}>
                        <div className="modal-content" style={{ borderRadius: '10px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.3)' }}>
                            <div className="modal-header" style={{ backgroundColor: '#f8f9fa', borderBottom: '1px solid #dee2e6', borderRadius: '10px 10px 0 0' }}>
                                <h5 className="modal-title">Award({schemeNames})-({tikitIDS})</h5>
                                <button
                                    type="button"
                                    className="btn-close"
                            ...
                            <div className="modal-body" style={{ padding: '20px' }}>
                                <form>
                                    <div className="row row-sm">
                                        ...
                                        <div className="col-sm-4 form-group">
                                            <label className="form-label">Inventory Maped</label>
                                            <select
                                                className="form-control select2"
                                                name="inventoryId"
                                                value={inventoryAllocation.inventoryId}
                                                onChange={handleChangeInvotorys}
                                                style={{ maxHeight: '200px', overflowY: 'auto' }} // Set height and enable scroll
                                            >
                                                <option value="">Select</option>
                                                {inventory.map((option) => {
                                                    ...
                                                    if (option.status === "Available") {
                                                        optionStyle = { color: "green" }; // Text color green
                                                    } else if (option.status === "Allocated") {
                                                        optionStyle = { color: "red" }; // Text color red
                                                    } else if (option.status === "Awarded") {
                                                        optionStyle = { color: "yellow" }; // Text color yellow
                                                    }
...
export default LdApprovedApplication
