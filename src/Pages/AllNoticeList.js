...
const AllNoticeList = () => {
    ...
    const fetchDataFromApi = async () => {
        setLoading(true);
        try {
            const { noticeType } = filterByObj;
            const url = `${apiUrl}/applicant/noticeLetterList?&noticeType=${noticeType}`;
                                                                        ...
                                                                        <tbody>
                                                                            {filteredUsers.map((user) => (
                                                                                <tr>
                                                                                    ...
                                                                                    <td>
                                                                                        Notice View:   {user && user.inventoryFollowUp && `${user.inventoryFollowUp.reminderType}` === 'Demand Notice' && (
                                                                                            <>
                                                                                                <Link to={`/demand-notice/${user && user.inventoryFollowUp && `${user.inventoryFollowUp.id}`}`} target="__blanck" title="remainder-payment-information">
                                                                                                    <i className="fe fe-eye me-3" style={{ cursor: 'pointer' }} />
                                                                                                </Link>
                                                                                            </>
                                                                                        )}
                                                                                        {user && user.inventoryFollowUp && `${user.inventoryFollowUp.reminderType}` === 'Final Demand Notice' && (
                                                                                            <>
                                                                                                <Link to={`/final-demand-notice/${user && user.inventoryFollowUp && `${user.inventoryFollowUp.id}`}`} target="__blanck" title="remainder-payment-information">
                                                                                                    <i className="fe fe-eye me-3" style={{ cursor: 'pointer' }} />
                                                                                                </Link>
                                                                                            </>
                                                                                        )}
                                                                                        {user && user.inventoryFollowUp && `${user.inventoryFollowUp.reminderType}` === 'Cancellation Notice' && (
                                                                                            <>
                                                                                                <Link to={`/cancellation-notice/${user && user.inventoryFollowUp && `${user.inventoryFollowUp.id}`}`} target="__blanck" title="remainder-payment-information">
                                                                                                    <i className="fe fe-eye me-3" style={{ cursor: 'pointer' }} />
                                                                                                </Link>
                                                                                            </>
                                                                                        )}
                                                                                    ...
                                                                                    <td>
                                                                                        {user && user.inventoryFollowUp && `${user.inventoryFollowUp.reminderType}`}
                                                                                    </td>
